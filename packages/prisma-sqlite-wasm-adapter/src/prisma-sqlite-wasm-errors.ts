...
export function convertError(error: unknown): never {
  if (error instanceof Error) {
    const code = (error as { resultCode?: number }).resultCode;
    const message = error.message;

    if (code !== undefined) {
      // Constraint violations — match on extended result codes (same as @prisma/adapter-libsql)
      ...
      // SQLITE_BUSY and all extended variants (261, 517, ...)
      if ((code & 0xff) === 5) {
        throw new DriverAdapterError({
          kind: 'SocketTimeout',
          originalMessage: message,
    ...
    // Message-based fallback for SQLITE_ERROR (code 1) and non-WASM errors
